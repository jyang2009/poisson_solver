...
cout<<"for defects"<<endl;
cout<<"E_defected=";
for(int i=0;i<19;i++)cout<<E_defected[i]<<"   ";
cout<<endl;
cout<<"E_perfect=";
for(int i=0;i<19;i++)cout<<E_perfect[i]<<"   ";
cout<<endl;
...
cout<<"E_MP=";
for(int i=0;i<10;i++)cout<<E_MP[i]<<"    ";
cout<<endl;
...
int solve_whole(VectorXd& x0,VectorXd& E_field,VectorXd& rho,VectorXd& concentration, double delta_Ev,int n_ele,double* mu,int n_region, double* l,double* h,int* dim,int* dim_start,int* n_defect,int* defect_start,double* E_CBM,double* E_VBM,double* volume,double* epsilon,double* DOS_deno,double* E_MP,int n_MP,int* charge,double* E_defected,double* E_perfect,int* delta_n,int* num_site,double* delta_F_vib,int* DOS_start,double* DOS,double* energy_DOS,int* ENCUT,double temp, int* group_site,double c_dopant1,double c_dopant2)
{
	...
	cout<<"region2 bulk"<<endl;
	i_region=n_region-1;
    generate_region(i_region,n_ele,n_MP,n_defect,defect_start,DOS_start,E_MP_region,E_MP,E_defect_region,E_defected,E_perfect_region,E_perfect,charge,charge_region,num_site_region,num_site,delta_F_vib_region,delta_F_vib,delta_n,delta_n_region,ENCUT,DOS_region,DOS,energy_DOS, energy_DOS_region, mu_region, mu,group_site,group_site_region);
	if(c_dopant2!=0)
	{
		    //find chemical potential for dopant that achieves the given concentration
			//find the total doping charge
			double Q_dopant;
			Q_dopant=c_dopant2*charge_region[n_defect[i_region]-1];
			cout<<"Q_dopant="<<Q_dopant<<endl;
			double * c_defect = new double[n_defect[i_region]];
			double * G_defect = new double[n_defect[i_region]];
			//desired Fermi level
			find_Ef_external(energy_DOS_region, DOS_region, ENCUT[i_region],DOS_deno[i_region], E_CBM[i_region], n_defect[i_region],n_ele, charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,&E_f2,group_site_region,Q_dopant);
				
				//cout<<"Ef="<<E_f<<endl;
				mu_l=-30.;
				mu_r=10.;
				mu_region[n_ele-1]=(mu_l+mu_r)/2.;
				while(abs(mu_l-mu_r)>1e-8)
				{
					
					defect_concentration(n_ele,n_defect[i_region],charge_region,delta_n_region,E_defect_region,E_perfect_region,num_site_region,E_MP_region,E_f2,temp,mu_region,E_VBM[i_region],delta_F_vib_region,c_defect,G_defect,group_site_region);
					//calculate G0 for dopant that achieves the desired concentration
					c_dopant_calc=0;
					for(int d=0;d<n_defect[i_region];d++){if(delta_n_region[d*n_ele+n_ele-1]==1){c_dopant_calc+=c_defect[d];}}
					if(c_dopant_calc<c_dopant2){mu_l=(mu_r+mu_l)/2.;}
					else{mu_r=(mu_r+mu_l)/2.;}
					cout<<"c_dopant_calc="<<c_dopant_calc<<"   mu="<<mu_region[n_ele-1]<<endl;
					mu_region[n_ele-1]=(mu_l+mu_r)/2.;
				}
			mu[i_region*n_ele+n_ele-1]=(mu_l+mu_r)/2.;
			mu[(i_region-1)*n_ele+n_ele-1]=(mu_l+mu_r)/2.;

			cout<<"Ef_external="<<E_f2<<endl;	
			mu_region[n_ele-1]=(mu_l+mu_r)/2.;
			double Q_external=0;
			defect_concentration(n_ele,n_defect[i_region],charge_region,delta_n_region,E_defect_region,E_perfect_region,num_site_region,E_MP_region,E_f2,temp,mu_region,E_VBM[i_region],delta_F_vib_region,c_defect,G_defect,group_site_region);
			for(int i=0;i<n_defect[i_region];i++)
			{
				Q_external+=c_defect[i]*charge_region[i];
			}
			cout<<"Q using Ef_external is"<<Q_external<<endl;
			find_Ef(energy_DOS_region, DOS_region, ENCUT[i_region],DOS_deno[i_region], E_CBM[i_region], n_defect[i_region],n_ele, charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,&E_f2,group_site_region);
			cout<<"Ef_internal="<<E_f2<<endl;
			...
			cout<<"Q using Ef_internal is"<<Q_external<<endl;
			delete[] c_defect;
			delete[] G_defect;
	...
	double beta=E_f2-E_f1+delta_Ev;
	
	//E_f is the reference value for bulk neutrality 
	...
	band_output<<E_f1<<"   "<<E_f2<<"    "<<beta<<"   "<<E_CBM[0]-E_VBM[0]<<"     "<<E_CBM[3]-E_VBM[3]<<endl;
	E_f2-=beta;//very important!! assures charge neutrality when V=beta

	//in this version, solve for four regions
	double E_f[n_region];
	E_f[0]=E_f1;
	E_f[1]=E_f1;
	E_f[2]=E_f2;
	...
	double delta_V_test;
	int num_step=0;
	double delta_V1_test;
	double delta_V1_left;
	double delta_V1_right;
	double delta_V2_test;
	double delta_V2_left;
	double delta_V2_right;
	double E1;
	double E2;
	...
	while((abs(delta_V_left-delta_V_right)>1e-3)&&num_step<max_step)
	{   
		//total voltage drop on left side
		delta_V_test=(delta_V_left+delta_V_right)/2;
		cout<<"delta_V_test="<<delta_V_test<<endl;
		
		delta_V1_left=0;
		delta_V1_right=delta_V_test;
		//voltage drop in region1
		while(abs(delta_V1_left-delta_V1_right)>1e-5)
		{
			
			delta_V1_test=(delta_V1_left+delta_V1_right)/2;
			cout<<"delta_V1_test="<<delta_V1_test<<endl;
			//solve for first region
			cout<<"solving region1...."<<endl;
		    i_region=0;
		    generate_region(i_region,n_ele,n_MP,n_defect,defect_start,DOS_start,E_MP_region,E_MP,E_defect_region,E_defected,E_perfect_region,E_perfect,charge,charge_region,num_site_region,num_site,delta_F_vib_region,delta_F_vib,delta_n,delta_n_region,ENCUT,DOS_region,DOS,energy_DOS, energy_DOS_region, mu_region, mu,group_site,group_site_region);
			flag_solve=solve_system(x0_region,n_ele,max_step,dim[i_region],h[i_region],0,delta_V1_test,volume[i_region],epsilon[i_region],energy_DOS_region,DOS_region,ENCUT[i_region],E_CBM[i_region], n_defect[i_region],charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,DOS_deno[i_region],E_f1,group_site_region);	
			cout<<"solved!!"<<endl;
			if(flag_solve==0){return 0;}
			E1=(delta_V1_test-x0_region(dim[i_region]-1))/h[i_region];
			for(int i=0;i<dim[i_region];i++)x0_solved(dim_start[i_region]+i)=x0_region(i);
			
			cout<<"solving regionc1..."<<endl;
			
		    i_region=1;
		    generate_region(i_region,n_ele,n_MP,n_defect,defect_start,DOS_start,E_MP_region,E_MP,E_defect_region,E_defected,E_perfect_region,E_perfect,charge,charge_region,num_site_region,num_site,delta_F_vib_region,delta_F_vib,delta_n,delta_n_region,ENCUT,DOS_region,DOS,energy_DOS, energy_DOS_region, mu_region, mu,group_site,group_site_region);
			flag_solve=solve_system(x0_region,n_ele,max_step,dim[i_region],h[i_region],delta_V1_test,delta_V_test,volume[i_region],epsilon[i_region],energy_DOS_region,DOS_region,ENCUT[i_region],E_CBM[i_region], n_defect[i_region],charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,DOS_deno[i_region],E_f1,group_site_region);	
			cout<<"solved!!"<<endl;
			if(flag_solve==0){return 0;}
			for(int i=0;i<dim[i_region];i++)x0_solved(dim_start[i_region]+i)=x0_region(i);
			E2=(-delta_V1_test+x0_region(0))/h[i_region];
            if(E1>E2){delta_V1_right=delta_V1_test;}
            else{delta_V1_left=delta_V1_test;}		
            cout<<"in 1, E1="<<E1<<"  E2="<<E2<<endl;	
		}
		delta_V2_left=delta_V_test-1.0;
		delta_V2_right=beta+1.0;
		while(abs(delta_V2_left-delta_V2_right)>1e-3)
		{
			delta_V2_test=(delta_V2_left+delta_V2_right)/2;
			//delta_V2_test=delta_V_test;
			cout<<"delta_V2_test="<<delta_V2_test<<endl;
			cout<<"solving region2..."<<endl;
			
		    i_region=3;
		    generate_region(i_region,n_ele,n_MP,n_defect,defect_start,DOS_start,E_MP_region,E_MP,E_defect_region,E_defected,E_perfect_region,E_perfect,charge,charge_region,num_site_region,num_site,delta_F_vib_region,delta_F_vib,delta_n,delta_n_region,ENCUT,DOS_region,DOS,energy_DOS, energy_DOS_region, mu_region, mu,group_site,group_site_region);
			flag_solve=solve_system(x0_region,n_ele,max_step,dim[i_region],h[i_region],delta_V2_test,beta,volume[i_region],epsilon[i_region],energy_DOS_region,DOS_region,ENCUT[i_region],E_CBM[i_region], n_defect[i_region],charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,DOS_deno[i_region],E_f2,group_site_region);	
			cout<<"solved!!"<<endl;
			if(flag_solve==0){return 0;}
			for(int i=0;i<dim[i_region];i++)x0_solved(dim_start[i_region]+i)=x0_region(i);
			E2=(-delta_V2_test+x0_region(0))/h[i_region];
			
			//solve for c2 region
			
			cout<<"solving regionc2...."<<endl;
		    i_region=2;
		    generate_region(i_region,n_ele,n_MP,n_defect,defect_start,DOS_start,E_MP_region,E_MP,E_defect_region,E_defected,E_perfect_region,E_perfect,charge,charge_region,num_site_region,num_site,delta_F_vib_region,delta_F_vib,delta_n,delta_n_region,ENCUT,DOS_region,DOS,energy_DOS, energy_DOS_region, mu_region, mu,group_site,group_site_region);
			for(int i=0;i<dim[i_region];i++){x0_region[i]=delta_V2_test;x0_solved(dim_start[i_region]+i)=delta_V2_test;}
			
			
			flag_solve=solve_system(x0_region,n_ele,max_step,dim[i_region],h[i_region],delta_V_test,delta_V2_test,volume[i_region],epsilon[i_region],energy_DOS_region,DOS_region,ENCUT[i_region],E_CBM[i_region], n_defect[i_region],charge_region, delta_n_region, E_defect_region,num_site_region,E_MP_region,temp,mu_region,E_perfect_region,E_VBM[i_region],delta_F_vib_region,DOS_deno[i_region],E_f2,group_site_region);	
			cout<<"solved!!"<<endl;
			if(flag_solve==0){return 0;}
			E1=(delta_V2_test-x0_region(dim[i_region]-1))/h[i_region];
			for(int i=0;i<dim[i_region];i++)x0_solved(dim_start[i_region]+i)=x0_region(i);
			
            if(E1>E2){delta_V2_right=delta_V2_test;}
            else{delta_V2_left=delta_V2_test;}	
            cout<<"in 2, E1="<<E1<<"  E2="<<E2<<endl;	
		...
		//write output
	   	...
	   	V_left[0]=0;V_left[1]=delta_V1_test;V_left[2]=delta_V_test;V_left[3]=delta_V2_test;
	    V_right[0]=delta_V1_test;V_right[1]=delta_V_test;V_right[2]=delta_V2_test;V_right[3]=beta;
		...
		for(int i=0;i<n_region;i++)
		{
			...
			{
				x0_region(j)=x0_solved(dim_start[i_region]+j);
			}
			...
			//for(int i=0;i<(dim[i_region]+1);i++)E_field(dim_start[i_region]+i_region+i)=E_field_region(i);
			//for(int i=0;i<dim[i_region];i++)rho(dim_start[i_region]+i)=rho_region(i);
			//for(int i=0;i<dim[i_region];i++)
			//{   concentration(concentration_start[i_region]+i)=concentration_region(i);
			//	concentration(concentration_start[i_region]+dim[i_region])=concentration_region(dim[i_region]+i);
			//	for(int j=0;j<n_defect[i_region];j++)
			//	concentration(concentration_start[i_region]+(j+2)*dim[i_region]+i)=concentration_region((j+2)*dim[i_region]+i);
			//}
		}
		
		if(Q_tot>0){delta_V_left=delta_V_test;}
		else{delta_V_right=delta_V_test;}
		cout<<"Q_tot="<<Q_tot;
   ...
   //write output
	...
	for(int i=0;i<n_region;i++)
	{
		...
		for(int k=0;k<dim[i_region];k++)x0_solved(dim_start[i_region]+k)=x0_region(k);
...
int solve_system(VectorXd& x0,int n_ele, int max_step,int dim,double h,double V_left, double V_right,double volume,double epsilon,double* energy_DOS, double* DOS, int ENCUT, double E_CBM, int n_defect, int* charge, int* delta_n, double* E_defected, int* num_site,double* E_MP, double temp,double* mu,double* E_perfect,double E_VBM,double* delta_F_vib,double DOS_deno,double E_f,int* group_site)
{
    ...
    */
	...
	while(((abs(y.maxCoeff())>1e-8||abs(y.minCoeff())>1e-8)&&num_itr<max_step)||num_itr==0)
	{
		num_itr+=1;
		//compute f_x0(x0)
